...
test('compares ts-pattern, SWC plugin, and plain JS columns after textarea edits', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: 'compiled ts-pattern module code' })).toContainText('export const renderWithTsPatternAsIs')
  await expect(page.getByRole('textbox', { name: 'compiled ts-pattern module code' })).toContainText('const html = match(result)')
