...
test('compares ts-pattern, SWC plugin, and plain JS columns after textarea edits', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: 'source ts-pattern code' })).toContainText("import { match, P } from 'ts-pattern'")
  await expect(page.getByRole('textbox', { name: 'source ts-pattern code' })).not.toContainText('type Data')
  await expect(page.getByRole('textbox', { name: 'compiled ts-pattern code' })).toContainText('export const renderWithTsPatternAsIs')
  await expect(page.getByRole('textbox', { name: 'compiled ts-pattern code' })).toContainText('const html = match(result)')
